 ...
 public class Surf implements ISurf {
	...
	public static Surf createInstance(BufferedImage img, float balanceValue,
		...
		if (instance == null) {
			...
			instance = new Surf(img, balanceValue, threshold, octaves);
	...
	public static Surf createInstance(BufferedImage img, float threshold, int octaves,
		...
		if (instance == null) {
			instance = new Surf(img, threshold, octaves, 0);
		} else {
			instance.setIntegralImage(img);
			instance.setOctaves(octaves);
			instance.setThreshold(threshold);
	 ...
	 */
	public Surf(BufferedImage img, float balanceValue, float threshold, int steps) {
	 ...
	 * Singleton method.
	 */
	public Surf(BufferedImage img, float threshold, int steps) {
		this.integralImage = new IntegralImage(img);
		this.imgHeight = img.getHeight();
		this.imgWidth = img.getWidth();
